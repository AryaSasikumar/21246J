...
int Base::encoder_reset(){
  LeftDrive.resetRotation();
  RightDrive.resetRotation();
  return SUCCESS;
}

double Base::get_encoder_rotation(){
...
void Base::left_spin(double velocity, vex::brakeType stop_type){
  if(velocity != 0.0){ 
    LeftDrive.spin(DEFAULT_DIRECTION_TYPE, velocity, DEFAULT_VEL_UNITS); 
  }else{ 
...
void Base::right_spin(double velocity, vex::brakeType stop_type){
  if(velocity != 0.0){ 
    RightDrive.spin(DEFAULT_DIRECTION_TYPE, velocity, DEFAULT_VEL_UNITS); 
  }else{ 
...
void Base::drive_stop(vex::brakeType break_type = DEFAULT_DRIVE_STOP_TYPE){
