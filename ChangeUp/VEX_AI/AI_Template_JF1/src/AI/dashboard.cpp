/*---DASHBOARD_CPP---*/
/*----------------------------------------------------------------------------*/
/*    Module:       dashboard.cpp                                             */
/*    Author:       Jeffrey Fisher II                                         */
/*    Created:      27 Dec 2020                                               */
/*----------------------------------------------------------------------------*/
#include "vex.h"

// Display various useful information about the Jetson
static void dashboardJetson( int ox, int oy, int width, int height ) {
  static int32_t     last_data = 0;
  static int32_t     last_packets = 0;
  static int32_t     total_data = 0;
  static int32_t     total_packets = 0;
  static uint32_t    update_time = 0;
  static MAP_RECORD  local_map;
  vex::color grey = vex::color(0x404040);

  Brain.Screen.setClipRegion( ox, oy, width, height);
  Brain.Screen.setFont( vex::mono15 );
  // border and titlebar
  Brain.Screen.setPenColor( vex::yellow );
  Brain.Screen.drawRectangle(ox, oy, width, height, vex::black );
  Brain.Screen.drawRectangle( ox, oy, width, 20, grey );

  Brain.Screen.setPenColor( vex::yellow );
  Brain.Screen.setFillColor( grey );
  Brain.Screen.printAt(ox + 10, oy + 15, "Jetson" );
  oy += 20;
  
  Brain.Screen.setPenColor( vex::white );
  Brain.Screen.setFillColor( vex::black );
  
  // get last map data
  jetson_comm.get_data( &local_map );

  // Brain.Screen.printAt( ox + 10, oy += 15, "Packets   %d", jetson_comm.get_packets() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "Errors    %d", jetson_comm.get_errors() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "Timeouts  %d", jetson_comm.get_timeouts() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "data/sec  %d             ", total_data );
  // Brain.Screen.printAt( ox + 10, oy += 15, "pkts/sec  %d             ", total_packets );
  // Brain.Screen.printAt( ox + 10, oy += 15, "boxnum    %d", local_map.boxnum );
  // Brain.Screen.printAt( ox + 10, oy += 15, "mapnum    %d", local_map.mapnum );

  // once per second, update data rate stats
  if( Brain.Timer.system() > update_time ) {
    update_time = Brain.Timer.system() + 1000;
    total_data = jetson_comm.get_total() - last_data;
    total_packets = jetson_comm.get_packets() - last_packets;
    last_data = jetson_comm.get_total();
    last_packets = jetson_comm.get_packets();
  }
  
  Brain.Screen.setFont( vex::mono12 );
  for(int i=0;i<4;i++ ) {
    if( i < local_map.boxnum ) {
      Brain.Screen.printAt( ox + 10, oy += 12, "box %d: x:%4d y:%4d w:%4d h:%4d d:%.1f",i,
                           local_map.boxobj[i].x,
                           local_map.boxobj[i].y,
                           local_map.boxobj[i].width,
                           local_map.boxobj[i].height,
                           local_map.boxobj[i].depth );
    }
    else {
      Brain.Screen.printAt( ox + 10, oy += 12, "---");
    }
  }
  for(int i=0;i<4;i++ ) {
    if( i < local_map.mapnum ) {
      Brain.Screen.printAt( ox + 10, oy += 12, "map %d: a:%4d c:%4d 0:%.2f 1:%.2f 2:%.1f",i,
                           local_map.mapobj[i].age,
                           local_map.mapobj[i].classID,
                           local_map.mapobj[i].p[0],
                           local_map.mapobj[i].p[1],
                           local_map.mapobj[i].p[2]);
    }
    else {
      Brain.Screen.printAt( ox + 10, oy += 12, "---");
    }
  }

}

//
// Display various useful information about VEXlink
//
static void dashboardVexlink( int ox, int oy, int width, int height ) {
  static int32_t last_data = 0;
  static int32_t last_packets = 0;
  static int32_t total_data = 0;
  static int32_t total_packets = 0;
  static uint32_t update_time = 0;  

  vex::color darkred = vex::color(0x800000);
  vex::color darkgrn = vex::color(0x008000);

  Brain.Screen.setClipRegion( ox, oy, width, height);
  Brain.Screen.setFont( vex::mono15 );

  // border and titlebar
  Brain.Screen.setPenColor( vex::yellow );
  Brain.Screen.drawRectangle(ox, oy, width, height, vex::black );
  Brain.Screen.drawRectangle( ox, oy, width, 20 );

  // Link status in titlebar
  if( robot_link.isLinked() ) {
    Brain.Screen.setPenColor(darkgrn);
    Brain.Screen.setFillColor(darkgrn);
    Brain.Screen.drawRectangle( ox+1, oy+1, width-2, 18 );
    Brain.Screen.setPenColor(vex::yellow);
    Brain.Screen.printAt( ox + 10, oy + 15, "VEXlink: Good" );
  }
  else {
    Brain.Screen.setPenColor(darkred);
    Brain.Screen.setFillColor(darkred);
    Brain.Screen.drawRectangle( ox+1, oy+1, width-2, 18 );
    Brain.Screen.setPenColor(vex::yellow);
    Brain.Screen.printAt(ox + 10, oy + 15, "VEXlink: Disconnected" );
  }
  oy += 20;

  Brain.Screen.setFillColor(vex::black);
  Brain.Screen.setPenColor(vex::white);
  // Brain.Screen.printAt( ox + 10, oy += 15, "Packets   %d", robot_link.get_packets() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "Errors    %d", robot_link.get_errors() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "Timeouts  %d", robot_link.get_timeouts() );
  // Brain.Screen.printAt( ox + 10, oy += 15, "data/sec  %d  ", total_data);
  // Brain.Screen.printAt( ox + 10, oy += 15, "pkts/sec  %d  ", total_packets);

  // once per second, update data rate stats
  if( Brain.Timer.system() > update_time ) {
    update_time = Brain.Timer.system() + 1000;    
    total_data = robot_link.get_total() - last_data;
    total_packets = robot_link.get_packets() - last_packets;
    last_data = robot_link.get_total();
    last_packets = robot_link.get_packets();
  }

  oy += 10;
  Brain.Screen.printAt( ox + 10, oy += 15, "Location: This Robot");
  
  float x,y,heading;
  robot_link.get_local_location(x, y, heading);
  
  Brain.Screen.printAt( ox + 10, oy += 15, " X:   %.2f", x);
  Brain.Screen.printAt( ox + 10, oy += 15, " Y:   %.2f", y);
  Brain.Screen.printAt( ox + 10, oy += 15, " H:   %.2f", heading);

  oy += 5;
  Brain.Screen.printAt( ox + 10, oy += 15, "Location: That Robot");
  
  robot_link.get_remote_location(x, y, heading);
  
  Brain.Screen.printAt( ox + 10, oy += 15, " X:   %.2f", x);
  Brain.Screen.printAt( ox + 10, oy += 15, " Y:   %.2f", y);
  Brain.Screen.printAt( ox + 10, oy += 15, " H:   %.2f", heading);
}

// Task to update screen with status
int dashboardTask(){
  while(FOREVER){
    dashboardJetson(    0, 0, 280, 240 );
    dashboardVexlink( 279, 0, 201, 240 );
    // draw, at 30Hz
    Brain.Screen.render();
    vex::this_thread::sleep_for(16);
  }
  return 0;
}