...
AI::Jetson::Jetson() {
  ...
  vex::thread t1 = vex::thread( receive_task, static_cast<void *>(this) );
  t1.setPriority(vex::thread::threadPriorityHigh);
}

AI::Jetson::~Jetson(){}

int32_t AI::Jetson::get_packets(){ return packets; } //Get # good received packets.

int32_t AI::Jetson::get_errors(){ return errors; } //Get # bad received packets.
 
int32_t AI::Jetson::get_timeouts(){ return timeouts; } //Get # timeouts triggered.
 
int32_t AI::Jetson::get_total(){ return total_data_received; } //Get # bytes received.

// The map record is copied to user supplied buffer
// length of the valid data is returned.
int32_t AI::Jetson::get_data(MAP_RECORD *map){ //Get the last received map record.
  int32_t length = 0;
  if( map != NULL ) {
    maplock.lock();
 ...
 */
uint32_t AI::Jetson::_crc32_table[256];

uint32_t AI::Jetson::crc32(uint8_t *pData, uint32_t numberOfBytes, uint32_t accumulator) {
  uint32_t i, j;
  const uint32_t POLYNOMIAL_CRC32 = 0x04C11DB7;
  if( _crc32_table[1] == 0 ){ // On first call generate the table
    uint32_t crc_accum;
    for(i = 0; i < 256; i++){
      crc_accum = i << 24;
      for(j = 0; j < 8; j++){
...
bool AI::Jetson::parse( uint8_t data ){ //Parse a single received byte
  bool  bRecall = false;
...
void AI::Jetson::request_map(){ //Send request to the Jetson to ask for next packet.
...
//Task to receive and process receive data from Jetson
int AI::Jetson::receive_task(void *arg){
    ...
    AI::Jetson *instance = static_cast<AI::Jetson *>(arg);
    ...
    while(FOREVER){
      ...
      if( rxchar >= 0 ){
        ...
        while(instance->parse((uint8_t)rxchar)){ vex::this_thread::yield(); }
      }
