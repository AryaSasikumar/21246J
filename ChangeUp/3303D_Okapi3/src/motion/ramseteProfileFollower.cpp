...
namespace RRLib
{
    ...
    RamseteGains::RamseteGains(double zeta, double b, double positionKD)
        : zeta(zeta), b(b), positionKD(positionKD) {}
    ...
    RamseteProfileController::RamseteProfileController(
        ...
        : gains(gains), headingkP(headingkP), model(model), poseEstimator(poseEstimator), constraints(constraints), scales(scales), ratioPair(ratioPair)
    {
